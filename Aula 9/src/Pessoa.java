...
public class Pessoa {
	...
	public int compareTo(Pessoa outraPessoa) {
		...
		// Caso contrário, comparo o nome das pessoas e se o nome da primeira for menor
		// que a segunda
		else if (this.nome.compareTo(outraPessoa.nome) < 0) {
			return -1; // Retornar -1, significa que o nome atual é anterior no alfabeto em relação a
						// outra Pessoa

			// Se não cair nos casos anteriores, retorna que é maior
		} else {
			return 1; // Retornar 1, significa que o nome atual é posterior no alfabeto em relação a
						// outra Pessoa.
		}
